...
class Metrics(object):
	def __init__(self, *args, **kwargs):
		...
		if self.metric_type == 'Accuracy':
			self.metric_object = Accuracy(**kwargs)
			...
			self.metric_object = Contrastive_Accuracy(**kwargs)
			...
			self.metric_object = Eval_PoseTrack18_det(**kwargs)
		elif self.metric_type == 'Eval_PoseTrack17_det':
			self.metric_object = Eval_PoseTrack17_det(**kwargs)
		else:
...
class Save_Video_Keypoints:
	...
	def __init__(self, *args, **kwargs):
		...
		self.eval_object = Eval_PoseTrack18_det(**kwargs)
	...
	def get_accuracy(self, predictions, data):
        ...
        list(range(predictions.shape[1]))
		...
		for b in range(B):
			...
			for t in range(T):
                ...
                _, _, _, _ = crop  # cropped input to model
	...
	def get_accuracy(self, predictions, data):
		...
		for b in range(B):
			...
			if self.prev_f_path == None:
				...
				frame_id = self.prev_f_path.split('/')[-1].split('.')[0]
                int(''.join(c for c in frame_id if c.isdigit()))

				# Normalize and quantize
				out_img = self.img / np.max(self.img)
                np.array(out_img * 255, dtype=np.uint8)
...
class Eval_PoseTrack18_det:
	...
	def assign_ids(self, json_anns, seq_name, image_id, keypoints, det_bboxes, frame_dims, frame, viz=False, vis_feats=None):
		if len(keypoints) == 0:
