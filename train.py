...
def train(**args):
    ...
    for _ in range(args['rerun']):
        ...
        result_dir = os.path.join(args['save_dir'], '_'.join((args['exp'], date)))
        ...
        for epoch in range(start_epoch, args['epoch']):
            acc_metric = Metrics(**args, ndata=len(train_loader.dataset), logger=None)
            running_loss = 0.0
            print('Epoch: ', epoch)
...
def valid(valid_loader, running_acc, model, device):
    0.
    acc_metric = Metrics(**args, ndata=len(valid_loader.dataset), logger=None)
    model.eval()
